...
const becameAVendor = [
  ...
  body("email", "Please enter valid email").exists().trim().isEmail().normalizeEmail(),
  ...
  body("images.*", "Please enter valid image url").optional().isObject(),
  body("video", "Please enter valid video").exists().trim().isObject(),
  body("avatar", "Please enter valid avatar").exists().trim().isObject(),
];
...
const updateVendor: any[] = [param("vendorId", "Please enter valid vendorId").exists().isMongoId(), ...becameAVendor];
