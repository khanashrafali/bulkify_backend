...
const isAuth = (uRoles) => (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let ReqData = req;
        ...
        let authUser;
        if (uRoles === null || uRoles === void 0 ? void 0 : uRoles.length) {
            if (uRoles.includes(interfaces_1.UserRole.USER)) {
                authUser = yield models_1.userModel.findOne({ _id: authData.id });
            }
            if (!authUser && uRoles.includes(interfaces_1.UserRole.VENDOR)) {
                authUser = yield models_1.vendorModel.findOne({ _id: authData.id });
            }
            if (!authUser && (uRoles.includes(interfaces_1.UserRole.ADMIN) || uRoles.includes(interfaces_1.UserRole.SUPER_ADMIN))) {
                authUser = yield models_1.adminModel.findOne({ _id: authData.id });
                if (authUser)
        ...
        if (!authUser)
            throw utils_1.helper.buildError(`Please Login Again!`, 401);
